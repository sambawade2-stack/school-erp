    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
        ...
        <div>
            ...
            @php
                ...
                $selectedIds = old('classe_ids', $classeIdsActuels);
            @endphp
            @if($classes->isEmpty())
                    ...
                    <div class="p-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <label class="flex items-center gap-1.5 px-3 py-1.5 border rounded-lg cursor-pointer text-sm transition-colors
                                {{ in_array($classe->id, $selectedIds) ? 'border-blue-500 bg-blue-50 text-blue-700 font-medium' : 'border-gray-200 text-gray-700 hover:border-blue-300 hover:bg-blue-50' }}"
                                id="label-classe-{{ $classe->id }}">
                                <input type="checkbox" name="classe_ids[]" value="{{ $classe->id }}"
                                    {{ in_array($classe->id, $selectedIds) ? 'checked' : '' }}
