@extends('layouts.app')
@section('titre', $examen->intitule)
@section('titre-page', 'Detail de l\'Examen')

@section('contenu')

<x-btn-retour :href="route('examens.index')" label="Retour aux examens" :breadcrumb="$examen->intitule" />

{{-- En-tête examen --}}
<div class="bg-blue-50 border border-blue-100 rounded-xl p-5 mb-5">
    <div class="flex items-start justify-between">
        <div>
            <h2 class="text-lg font-bold text-blue-800">{{ $examen->intitule }}</h2>
            <div class="flex flex-wrap gap-4 mt-2 text-sm text-blue-600">
                <span class="flex items-center gap-1">
                    ...
                    <strong>{{ $examen->matiere->nom }}</strong>
                </span>
                <span class="flex items-center gap-1">
                    ...
                    Classe <strong>{{ $examen->classe->nom }}</strong>
                </span>
                <span class="flex items-center gap-1">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    {{ $examen->date_examen->format('d/m/Y') }}
                </span>
                <span>Note max : <strong>{{ $examen->note_max }}</strong></span>
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-200 text-blue-800">
                    {{ $examen->trimestre }} — {{ $examen->annee_scolaire }}
                </span>
            </div>
        </div>
        <div class="flex gap-2 flex-shrink-0">
            <a href="{{ route('notes.saisir', $examen) }}"
               style="background:#2563eb; color:#fff; display:inline-flex; align-items:center; gap:8px; padding:8px 16px; border-radius:8px; font-size:0.875rem; font-weight:600; text-decoration:underline; text-underline-offset:3px;">
...
{{-- Statistiques --}}
@php
    $notesValues = $examen->notes->pluck('note');
    ...
    $nbReussi = $examen->notes->where('note', '>=', $examen->note_max / 2)->count();
@endphp
...
<div class="grid grid-cols-4 gap-4 mb-5">
    ...
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 text-center">
        ...
        <p class="text-2xl font-bold {{ $moyenne >= $examen->note_max / 2 ? 'text-green-600' : 'text-red-500' }}">
            {{ $moyenne }}/{{ $examen->note_max }}
        </p>
    </div>
    <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-4 text-center">
...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50">
        ...
        @if($nbNotes === 0)
        <a href="{{ route('notes.saisir', $examen) }}"
           style="background:#2563eb; color:#fff; padding:6px 14px; border-radius:8px; font-size:0.78rem; font-weight:600; text-decoration:underline; text-underline-offset:3px;">
            Saisir les notes
        </a>
        @endif
    </div>
    ...
    <table class="w-full text-sm">
        <thead class="border-b border-gray-100">
            <tr>
                ...
                <th class="px-5 py-3 text-center text-xs font-semibold text-gray-500 uppercase">/ {{ $examen->note_max }}</th>
        ...
        <tbody class="divide-y divide-gray-50">
            @php
                $notesParEtudiant = $examen->notes->keyBy('etudiant_id');
            @endphp
            @foreach($etudiants as $i => $etudiant)
            @php
                $note = $notesParEtudiant[$etudiant->id] ?? null;
                $reussi = $note && $note->note >= ($examen->note_max / 2);
            @endphp
            <tr class="hover:bg-gray-50 transition-colors">
                ...
                <td class="px-5 py-2.5 text-center text-gray-400 text-xs">{{ $examen->note_max }}</td>
                <td class="px-5 py-2.5 text-center">
                    @if($note)
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium
                        {{ $note->note >= 16 ? 'bg-green-100 text-green-700' :
                          ($note->note >= 14 ? 'bg-teal-100 text-teal-700' :
                          ($note->note >= 12 ? 'bg-blue-100 text-blue-700' :
                ...
                <td class="px-5 py-2.5">
                    ...
                    <div class="w-full bg-gray-100 rounded-full h-1.5">
                        <div class="h-1.5 rounded-full {{ $reussi ? 'bg-green-400' : 'bg-red-400' }}"
                             style="width: {{ min(100, ($note->note / $examen->note_max) * 100) }}%"></div>
                    </div>
                    @endif
                </td>
            </tr>
