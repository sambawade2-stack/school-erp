@extends('layouts.app')
@section('titre', 'Nouvel Examen')
@section('titre-page', 'Creer un Examen')

@section('contenu')

<x-btn-retour :href="route('examens.index')" label="Retour aux examens" breadcrumb="Nouvel examen" />
...
<form action="{{ route('examens.store') }}" method="POST">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="date_examen" value="{{ old('date_examen') }}" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none">
            </div>
            <div>
    ...
    <div class="flex gap-3 mt-5">
        <button type="submit" class="px-6 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Creer l'examen</button>
